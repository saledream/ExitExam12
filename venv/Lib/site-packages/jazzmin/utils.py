...
def get_db_objects():
    users = apps.get_registered_model("accounts","User").objects.all().count()
    courses = apps.get_registered_model("EECommittee",'Course').objects.all().count() 
    departments = apps.get_registered_model("EECommittee",'Department').objects.all().count() 
    modules = apps.get_registered_model("instructor",'Module').objects.all().count() 
    modelExams = apps.get_registered_model('instructor',"Exam_Model").objects.all().count()
    testExam = apps.get_registered_model('instructor','Test').objects.all().count() 
    testQ = apps.get_registered_model("instructor","TestQuestion").objects.all().count()
    modelQ = apps.get_registered_model("instructor","ModelQuestion").objects.all().count() 
    progress = apps.get_registered_model("instructor",'CourseProgress').objects.all().count() 

    return {"users":users, "courses":courses,"departments": departments,"modules":modules,"models": modelExams,"tests": testExam,"totalModelQ":modelQ,"totalTestQ":testQ,"progress":progress} 
...
def get_admin_url(instance: Any, admin_site: str = "admin", from_app: bool = False, **kwargs: str) -> str:
    ...
    """
    url = "#"
    print("this is sirous issue!! beya!!@")
...
def get_app_admin_urls(app: str, admin_site: str = "admin") -> List[Dict]:
    ...
    for model in apps.app_configs[app].get_models():
        ...
        print("this is sirous issue!!!!@")
        print(apps.get_registered_model())
